...
- **min_peer_proto_version (Optional):** This is the oldest/lowest protocol version that is allowed to communicate with nodes on the blockchain network. **Ex:** 70015. Typically you can find the minimum peer protocol version in your coins `version.h` file. Leave this value blank or set it to the same value as `protocol_version` if this setting does not exist in your blockchain. Nodes below this value are never treated as good: they are not published and not listed in the node API. They are dropped and tested again after `outdated_peer_retry_seconds`, so a node that upgrades comes back within that time. **Ex:** 70242 (DeFCoN v23).
- **outdated_peer_retry_seconds (Optional):** How long a node below `min_peer_proto_version` stays dropped before the crawler tests it again. Default 3600 (1 hour), minimum 60. Upstream hard-coded 7 days, which hid nodes that upgraded during an upgrade window for up to a week.
...
- **cf_api_key (Optional):** A Cloudflare **API token**, sent as `Authorization: Bearer` by `cf-uploader/cf.py` — not the Global API Key. Create it under My Profile → API Tokens → Create Token → Create Custom Token with exactly two permissions, **Zone → Zone → Read** and **Zone → DNS → Edit**, limited to the one zone in `cf_domain`; restricting it to the seeder host's IP is recommended. Anyone holding it can repoint the seed DNS name, so keep it only in `settings.conf` (which is git-ignored, as are its backups) and rotate it if it is ever exposed. This value is only required if you followed the [Cloudflare Setup](#cloudflare-setup).
- **required_subversion (Optional):** Only nodes whose user agent contains this string are published. **Ex:** `/DeFCoN:`. Applied by the uploader only; the crawler is not affected.
- **max_blocks_behind (Optional):** Drop candidates more than this many blocks below the highest candidate. **Ex:** 6.
- **max_seed_age (Optional):** Drop nodes whose last successful contact is older than this many seconds. **Ex:** 10800.
- **require_node_network (Optional):** When `1`, publish only nodes that advertise `NODE_NETWORK`.
- **publish_min_proto_version (Optional):** Publish only nodes whose protocol version is at least this value. Falls back to `min_peer_proto_version` when unset. Applied by the uploader only, so unlike `min_peer_proto_version` it does not make the crawler ban anyone: a node that upgrades is seen on the crawler's next visit and published on the next upload run. **Ex:** 70242 (DeFCoN v23, required from block 144768).
